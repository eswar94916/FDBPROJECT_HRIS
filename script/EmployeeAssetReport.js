...
function showDetails() {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify({ 'compid': $('#ddlcompany').val(), 'datefrom': datef, 'dateto': dateto, 'unit': unit, 'dept': dept, 'assettype': asset }),
        url: "EmployeeAssetReportServices.aspx/empassetreport",
        dataType: "json",
        async: true,
        success: function (data, status) {
            var varList = (data.d);
            ...
            if (varList.length == 0) {
                ...
                $(varList).each(function (index, o) {
                    ...
                    t.row.add(['<label>' + serial_no + '<label>', '<label>' + o.UnitName + '<label>', '<label>' + o.EmpCode + '<label>', '<label>' + o.EmpName + '<label>', '<label>' + o.DepartmentName + '<label>', '<label>' + o.DesignationName + '<label>', '<label>' + o.CompAssetsHouse + '<label>', '<label>' + o.CompAssetsCar + '<label>', '<label>' + o.CompAssetsLaptop + '<label>', '<label>' + o.CompAssetsMobile + '<label>', '<label>' + o.CompAssetsDongle + '<label>', '<label>' + o.CompAssetsCUG + '<label>', '<label>' + o.CompAssetsOthers + '<label>']).draw();
                });
            }
            $('#sample_1').show();
            $('#divdetails').show();
...
function showDetails1() {
    ...
    $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify({ 'compid': $('#ddlcompany').val(), 'datefrom': datef, 'dateto': dateto, 'unit': unit, 'dept': dept, 'assettype': asset }),
        url: "EmployeeAssetReportServices.aspx/empassetreportfilter",
        dataType: "json",
        async: true,
        success: function (data, status) {
            var varList = (data.d);
            var hname = $("#ddlassets :selected").text();
            $('#thname').html(hname);
            ...
            if (varList.length == 0) {
                ...
                $(varList).each(function (index, o) {
                    serial_no = serial_no + 1;
                    t.row.add(['<label>' + serial_no + '<label>', '<label>' + o.UnitName + '<label>', '<label>' + o.EmpCode + '<label>', '<label>' + o.EmpName + '<label>', '<label>' + o.DepartmentName + '<label>', '<label>' + o.DesignationName + '<label>', '<label>' + o.AssestValue + '<label>']).draw();
                });
            }
            $('#sample_2').show();
            $('#divdetails').hide();
