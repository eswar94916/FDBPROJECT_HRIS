...
function showDetails() {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify({ 'unit_id': unitid, 'depid': depid }),
        url: "EmployeeSignUpServices.aspx/DocumentStatusList",
        dataType: "json",
        async: true,
        success: function (data, status) {
            var varList = (data.d);
            ...
            if (varList.length == 0) {
                ...
                $(varList).each(function (index, o) {
                    ...
                    t.row.add(['<label>' + serial_no + '<label>', '<label >' + o.UnitName + '<label>', '<label >' + o.EmployeeCode + '<label>', '<label >' + o.EmployeeName + '<label>', '<label >' + o.DepartmentName + '<label>', '<label >' + o.DesignationName + '<label>', '<label >' + o.PhotoPath + '<label>', o.ResumePath, '<label >' + o.MarksheetPath + '<label>', o.RefLetterPath, o.ChequePath,o.PanCardPath,o.AddressProofPath ]).draw();
                });
            }
            $('#sample_2').show();
            $('#divdetails').show();
